...
class BaseController {
    ...
    private function displayLoginAndMenu() {
        ...
        if ($loggedInUser != null) {
            ...
            $this->template->assign('menuEntries', array('accounts','password-generator'));
        } else {
    ...
    public function displayPWGenerator(Password $password = null) {
        if ($this->getLoggedInUser()) {
            ...
            $this->template->assign('activeMenu', 'password-generator');
    ...
    public function generatePassword(Password $password) {
        if ($this->getLoggedInUser()) {
            ...
            $faults = $passwordValidator->validate($password);
            if (!$faults) {
                $chars = '';
                ...
                for ($i = 0; $i < 10; $i++) {
                    $generatedPasswords[] = substr(str_shuffle($chars), 0, $password->getLength());
                }

                $this->template->assign('generatedPasswords', $generatedPasswords);
            } else {
